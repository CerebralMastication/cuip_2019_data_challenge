...
ok, so we're seeing some lagged correlation. It peaks at .06 of a frequency unit... our frequency is 6 * 24 * 7, so the lag is 6 * 24 * 7 * .06 = `r 6 * 24 * 7 * .06`

```{r eval=FALSE, include=FALSE}
single_intersect_ts_lagged <-
  merge(single_intersect_ts,
        stats::lag(single_intersect_ts[, "total_traffic"], k = -60, na.pad = TRUE)
        )

## pick up here...

lm_fit <- tslm(pm2_5_cf_1 ~ x, data=single_intersect_ts_lagged)
summary(lm_fit)
# 
