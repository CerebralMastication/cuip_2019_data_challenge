...
This example script is `index.Rmd` which is rendered to html and served via this project's GitHub pages site:  https://cerebralmastication.github.io/cuip_2019_data_challenge/
...
We're supposed to be looking at PM 2.5. What is PM 2.5? Well I didn't know so I googled that junk. Came up with this helpful link:
...
Oh... the data is irregular. Ok, so we need to put this junk in buckets. I'm thinking maybe 10 min buckets? Let's see if we can do that. Feels like a job for grouping by a floor value. We can get the 10 minute interval floors from the `lubridate` package. 
...
There's not enough data here to be meaninful, so I put zero value on these correlation measures. We can come back later with more data and take a look at these. 
